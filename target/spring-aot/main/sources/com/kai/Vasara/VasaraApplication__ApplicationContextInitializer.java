 ...
 */
@Generated
public class VasaraApplication__ApplicationContextInitializer implements ApplicationContextInitializer<GenericApplicationContext> {
  @Override
  public void initialize(GenericApplicationContext applicationContext) {
    DefaultListableBeanFactory beanFactory = applicationContext.getDefaultListableBeanFactory();
    beanFactory.setAutowireCandidateResolver(new ContextAnnotationAutowireCandidateResolver());
    beanFactory.setDependencyComparator(AnnotationAwareOrderComparator.INSTANCE);
    applicationContext.getEnvironment().addActiveProfile("native");
    addImportAwareBeanPostProcessors(beanFactory);
    new VasaraApplication__BeanFactoryRegistrations().registerBeanDefinitions(beanFactory);
    new VasaraApplication__BeanFactoryRegistrations().registerAliases(beanFactory);
  }
  ...
  private void addImportAwareBeanPostProcessors(DefaultListableBeanFactory beanFactory) {
    ...
    mappings.put("org.springframework.session.jdbc.config.annotation.web.http.JdbcHttpSessionConfiguration", "org.springframework.boot.autoconfigure.session.JdbcSessionConfiguration");
